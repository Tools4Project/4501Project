...
>>>>>>> 4a811023b5065b139a7064fb1112dfa5142dcb68:Gui_main_page.py
...
def hit_me1():
<<<<<<< HEAD:Add_Error_Message.py
    File1 = filedialog.askopenfilename(parent=window, initialdir="/", title='Choose an image.')
    #show error message when uploading a file that is not a picture
    if File1.lower().endswith('jpg') or File1.lower().endswith('jpeg') or File1.lower().endswith('png') or File1.lower().endswith('bmp'):
        picture1 = ImageTk.PhotoImage(Image.open(File1).resize((300,250)))
        canvas.image = picture1  # <--- keep reference of your image
        canvas.create_image(50, 1, anchor='n', image=picture1)
    else:
...
def upload_pic():
<<<<<<< HEAD:Add_Error_Message.py
    File2 = filedialog.askopenfilename(parent=window, initialdir="/", title='Choose an image.')
    #show error message when uploading a file that is not a picture
    if File2.lower().endswith('jpg') or File2.lower().endswith('jpeg') or File2.lower().endswith('png') or File2.lower().endswith('bmp'):
        picture2 = ImageTk.PhotoImage(Image.open(File2).resize((300,250)))
        canvas.image2 = picture2 # <--- keep reference of your image
        canvas.create_image(50, 1, anchor='n', image=picture2)
    else:
        tkinter.messagebox.showinfo(title='Error', message="Please try to upload a 'jpg','jpeg','png','bmp' file") 
=======
    File = filedialog.askopenfilename(parent=window, initialdir="./", title='Choose an image.')
...
def cnn_paint_style():
    ...
    ## Inputs
    file_content_image = pic_fp
    file_style_image = art_fp
